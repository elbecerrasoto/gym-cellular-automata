...
class Designator(Operator):

    grid_dependant = True
    action_dependant = True
    context_dependant = False
    ...
    def update(self, grid, action, context):
        ...
        (row, col), (from_cell, to_cell) = position, effect = action

        target = grid[row, col]
        if target == from_cell:
            grid[row, col] = to_cell

        return grid, context

class CoordinatorBulldozer(Operator):
    ...
    def __init__(self, designator, move, *args, **kwargs):
    ...
    def update(self, grid, action, context):
        ...
        ca_params, bull_position, current_time, ti  = context
        amove, ashoot = action

        action = [] 
        position, change = context
        row, col = position
        from_cell, to_cell = change 
