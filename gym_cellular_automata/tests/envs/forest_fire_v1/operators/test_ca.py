...
def random_grid():
    ...
    cell_values = np.array([ca.EMPTY, ca.TREE, ca.FIRE])
...
@pytest.fixture
def uniform_space():
    return spaces.Box(0.0, 1.0, shape=(ca.ROW_K, ca.COL_K))
...
def test_kernel_generation(deterministic_winds, uniform_space):
    ...
    assert check_kernel_weight_at_position(
        all_propagating_kernel, (0, 0), ca.PROPAGATION
    )

    assert check_kernel_weight_at_position(
        all_propagating_kernel, (0, 0), ca.EMPTY
